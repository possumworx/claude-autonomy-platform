...
INFRA_CONFIG = Path("claude_infrastructure_config.txt")
LOG_FILE = Path("logs/channel_monitor.log")
    ...
    try:
        if INFRA_CONFIG.exists():
            with open(INFRA_CONFIG, 'r') as f:
                for line in f:
                    if line.startswith('CHANNEL_MONITOR_HEALTHCHECK_URL='):
                        config["healthcheck_url"] = line.split('=', 1)[1].strip()
                        break
                    # Also check for the old name
