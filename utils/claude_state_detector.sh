...
wait_for_claude_ready() {
    ...
    if [ -z "$claude_pane" ]; then
        echo "[DETECTOR] Warning: Could not find Claude pane"
        return 1
    fi
    ...
    while [ $count -lt $max_wait ]; do
        ...
        if ! echo "$pane_content" | grep -qE '^[.+*❄✿✶]|…'; then
            echo "[DETECTOR] Claude is ready after ${count}s"
    ...
    echo "[DETECTOR] Warning: Claude may still be thinking after ${max_wait}s"
    return 1
}