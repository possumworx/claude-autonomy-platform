...
# Captures context, backs up work, cleanly halts ClAP
...
KEYWORD=${1:-"NONE"}
echo "[SESSION_SWAP] ClAP Halting"
log_info "SESSION_SWAP" "Ending ClAP cleanly"
log_swap_event "SWAP_START"  "initiated" "Session wind up initiated by user"
...
echo "[SESSION_SWAP] Notifying Claude that wrap-up is starting..."
send_to_claude "🔄 Session wrap-up triggered. Please stay quiet while the process completes..."
...
if [[ -f "$full_path" ]]; then
    ...
    if [[ $time_diff -le 30 ]]; then
        ...
        # Alert Claude that swap failed
        send_to_claude "❌ Session wrap-up abandoned: Export file not recently modified. Still in current session."
        sleep 2
    ...
    # Alert Claude that swap failed
    send_to_claude "❌ Session wrap-up abandoned: Export file not created. Still in current session."
    sleep 2
...
# Remove lockfile
echo "[SESSION_SWAP] Removing lockfile..."
rm -f "$LOCKFILE"
...
log_swap_event "SWAP_COMPLETE" "$KEYWORD" "success" "Session wrap-up completed successfully"
log_info "SESSION_SWAP" "Session wrap-up completed successfully"
